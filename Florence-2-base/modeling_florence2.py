    ...
    def __init__(self, config: Florence2LanguageConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        if config.compression_mode != "none":
            ...
            if config.compression_mode == "avg_pool":
                self.pooling_operator = compress_average_pooling(self.pooling_factor)
            elif config.compression_mode == "learnable_pool":
                self.pooling_mlp = Learnable_pooling(self.pooling_factor, sorted = config.compression_sorted)
                self.pooling_operator = self.pooling_mlp.forward
            else:
                raise ValueError(f"Unsupported compression mode: {config.compression_mode}")
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.compression_mode != "none" and idx == self.grouping_layer:
                ...
                # hidden_states shape after compression: (batch_size, new_seq_len, d_model)
                # new_seq_len = seq_len // pooling_factor
